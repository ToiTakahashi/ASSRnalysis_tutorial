...
%% Multi
ms_dpd = zeros(n_dur, n_reprs);
ms_csm = zeros(n_dur, n_reprs);
%ms_mi = zeros(n_dur, n_reprs);

skip_dur = false(n_dur, 1);
...
for repr_idx = 1:n_reprs
    ...
    %assert(n_subject >= 2)
    if n_subject < 2 || skip_dur(dur_idx)
        disp('skip')
        skip_dur(dur_idx) = true;
        continue
    end
    ...
    ms_dpd(dur_idx, repr_idx) = metric_repr_dpd;
    ...
    ms_csm(dur_idx, repr_idx) = metric_repr_csm;
    ...
    ms_mi(dur_idx, repr_idx) = metric_repr_mi;
    %}
...
% assume same f_repr
fig_title = ['ACC-fs' num2str(data_fs) '-MF' num2str(analysis_freq) '-CF' num2str(f_reprs(1))];
ms_1 = struct('ms', ms_csm(~skip_dur, :), 'type', 'CSM');
ms_2 = struct('ms', ms_dpd(~skip_dur, :), 'type', 'DPD');
compare_accs(analysis_durs(~skip_dur)', l_reprs, ms_1, ms_2, fig_title, fig_dir)
