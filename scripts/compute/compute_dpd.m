function data_struct = compute_dpd(eeg_raw, fs, compute_opts)
...
dp_hist_ii = cell(clip_shift_pattern, n_column);
dp_hist_dd = cell(clip_shift_pattern, n_column);
dp_hist_id = cell(clip_shift_pattern, n_column);
...
for i = 1:n_column
    parfor csp = 1:clip_shift_pattern
        ...
        [dp_mat_ii, dp_mat_dd, dp_mat_id] = dpd(data_mat, use_matmul);

        [dp_center_column, dp_hist_column_ii, dp_hist_column_dd, dp_hist_column_id, dp_hist_item_min]...
            = adjusted_hist(dp_mat_ii, dp_mat_dd, dp_mat_id, hist_div, hist_scaler, hist_opt);

        dp_center{csp, i} = dp_center_column;
        dp_hist_ii{csp, i} = dp_hist_column_ii;
        dp_hist_dd{csp, i} = dp_hist_column_dd;
        dp_hist_id{csp, i} = dp_hist_column_id;

        csp_cur_val_div = evaluate_dp_div(dp_center_column, dp_hist_column_ii, dp_hist_column_dd, dp_hist_column_id, dp_hist_item_min, eval_opt);
...
data_struct.dp_hist_ii = dp_hist_ii;
data_struct.dp_hist_dd = dp_hist_dd;
data_struct.dp_hist_id = dp_hist_id;
