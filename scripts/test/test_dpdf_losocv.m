function [metric_repr, best_ms, best_threshs, confu] = test_dpdf_losocv(repr, sil, subject_r_idxs, subject_s_idxs, test_opt, threshs)
...
for s = 1:n_subject
    ...
    [~, ~, ~, ms] = test_dpdf(repr_rem, sil_rem, test_opt, threshs);
    ...
    for rl = 1:n_rl
        test_stat_1(rl) = test_stat_dpdf(repr_loo{rl}, test_opt);
    end
    ...
    for sl = 1:n_sl
        test_stat_0(sl) = test_stat_dpdf(sil_loo{sl}, test_opt);
    end
