function [metric_repr, best_ms, best_threshs, confu] = test_mi_losocv(repr, sil, subject_r_idxs, subject_s_idxs, test_opt, threshs)
...
for s = 1:n_subject
    ...
    [~, ~, ~, ms] = test_mi(repr_rem, sil_rem, test_opt, threshs);
    ...
    for rl = 1:n_rl
        test_stat_1(rl) = test_stat_mi(repr_loo{rl}, test_opt);
    end
    ...
    for sl = 1:n_sl
        test_stat_0(sl) = test_stat_mi(sil_loo{sl}, test_opt);
    end
